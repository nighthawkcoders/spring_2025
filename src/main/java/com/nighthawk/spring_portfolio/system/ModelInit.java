    ...
    CommandLineRunner run() {  // The run() method will be executed after the application starts
        return args -> {
            ...
            for (Person person : personArray) {
                // Name and uid are used to lookup the person
                List<Person> personFound = personDetailsService.list(person.getName(), person.getUid());  // lookup
                if (personFound.size() == 0) { // add if not found
                    ...
                    String text = "Test " + person.getUid();
